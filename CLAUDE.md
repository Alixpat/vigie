...
- **`train/`** — covers the pure helpers extracted from `TrainFragment` (`IncidentClassifier`, `LineNDirection`, `IdfmClient`, `TrainPosition`, `JourneyRoutes`, `OngoingTrains`). Touching keyword arrays, direction matching, URL construction, or the ongoing-train selection rules without running tests is asking for a regression.
...
- **`OngoingTrains`** — sélection des trains « qui me concernent à l'instant T » (section *En circulation sur mon trajet*). `isOngoing`/`isUpcoming` partagent la liste des départs en deux ; `describe` produit l'`OngoingTrain` prêt à afficher. `buildOngoing` s'appuie **d'abord sur le parcours** (`journeyStopsCache`, fusionné par `JourneyRoutes`) : il porte mes deux gares, donc l'heure de départ, l'heure d'arrivée et le sens de circulation — celui-ci se lit dans l'ordre des arrêts (`indexOfStop(origine) < indexOfStop(destination)`), sans deviner le terminus. `indexOfStop` retombe sur l'identifiant numérique de la gare (`LineNDirection.getOriginStopId` / `getDestinationStopId`) quand les noms d'arrêts n'ont pas été résolus. Le stop-monitoring des deux gares ne fait plus qu'enrichir (retard réel, voie, numéro de train) et sert de secours quand le parcours est inconnu — s'y fier pour la sélection ne montrait le train qu'en toute fin de trajet, puisqu'un train parti disparaît du stop-monitoring de ma gare de départ et n'entre dans celui de ma gare d'arrivée que dans l'horizon publié par IDFM. `rememberOriginVisits` mémorise en plus les passages vus à ma gare de départ aux rafraîchissements précédents (3 h). Attention : `effectiveDepartureMillis` lit `TrainSchedule.getExpectedDepartureMillis()` (heure réelle de départ, renseignée à la construction) et ne retombe sur `aimed + delayMinutes` que si elle vaut 0 — un retard pris en route ne doit pas décaler l'heure à laquelle le train est parti, sans quoi un train déjà en marche repasse pour « pas encore parti ». Le parcours arrivant après les horaires au premier chargement, `TrainFragment.rebuildOngoingFromLastFetch` rejoue la construction après chaque estimated-timetable.
