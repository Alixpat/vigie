...
- **`train/`** — covers the pure helpers extracted from `TrainFragment` (`IncidentClassifier`, `LineNDirection`, `IdfmClient`, `SegmentLeg`, `TrainPosition`, `OngoingTrains`). Touching keyword arrays, direction matching, URL construction, or the ongoing-train selection rules without running tests is asking for a regression.
...
- **`SegmentLeg`** — « ce train dessert-il mes deux gares, et dans quel sens ? ». `resolve(originVisit, destinationVisit, stops, direction)` fusionne trois sources (stop-monitoring des deux gares + parcours estimated-timetable) et expose les horaires/retards aux deux extrémités ; `servesMySegment()` exige les deux arrêts **dans l'ordre du sens testé**, ce qui détermine la direction sans regarder le terminus. C'est le critère utilisé par `buildCrossReferencedSchedules` et par `OngoingTrains` — `LineNDirection.matchesDestination` n'est plus qu'un repli quand on ne connaît rien du parcours (ni visite à la gare d'arrivée, ni arrêts en cache). Ne pas revenir au filtrage par mots-clés : il écarte des trains bien réels (un Paris → *Épône - Mézières* dessert Villepreux et ne matche aucun mot-clé).
- **`OngoingTrains`** — sélection des trains « qui me concernent à l'instant T » (section *En circulation sur mon trajet*). `isOngoing`/`isUpcoming` partagent la liste des départs en deux ; `buildOngoing(Sources, direction, now)` reconstruit les trains déjà partis en balayant l'union des journeyRef des trois sources (`Sources` = visites des deux gares + `journeyStopsCache` + caches numéro/mission) — le parcours estimated-timetable est la seule source qui connaisse encore un train parti ; `describe` produit l'`OngoingTrain` prêt à afficher. Attention : `effectiveDepartureMillis` n'applique le retard au départ que si une heure de départ estimée est renseignée — un retard pris en route ne change pas l'heure à laquelle le train est parti.

Les horaires sont ramenés sur une fenêtre `[now - 30 min, now + 2 h]` : `OngoingTrains.selectUpcoming` alimente les deux listes de départs, le reste bascule dans la section *En circulation*. Un tick local (`POSITION_TICK_MS`, 20 s) rejoue ce partage, **reconstruit les trains en circulation** depuis les dernières réponses brutes (`lastClamartVisits` / `lastVillepreuxVisits`, publiées `volatile` par le thread de fond) et recalcule les positions — le tout sans appel réseau ; seul `fetchSchedules` (5 min) refait les requêtes. `parseEstimatedTimetable` déclenche aussi un `updateOngoingSection()` : le parcours complet arrive après les horaires et complète la liste.

Les caches partagés du fragment (`journeyStopsCache`, `journeyTrainNumberCache`, `journeyMissionNameCache`, `stopPointNameCache`) sont des `ConcurrentHashMap` : le thread de fond les remplit pendant que le thread UI les parcourt à chaque tick.
