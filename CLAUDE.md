...
- **`OngoingTrains`** — sélection des trains « qui me concernent à l'instant T » (section *En circulation sur mon trajet*). `isOngoing`/`isUpcoming` partagent la liste des départs en deux ; `buildOngoing` reconstruit les trains déjà partis à partir du stop-monitoring de la gare **d'arrivée** (un train parti a disparu de celui de la gare de départ) et de `journeyStopsCache` pour retrouver l'heure de départ de ma gare ; `describe` produit l'`OngoingTrain` prêt à afficher. Attention : `effectiveDepartureMillis` n'applique le retard au départ que si une heure de départ estimée est renseignée — un retard pris en route ne change pas l'heure à laquelle le train est parti. Trois secours servent à retrouver l'heure de départ d'un train déjà parti (elle n'est plus dans le stop-monitoring de la gare de départ) : `rememberOriginVisits` mémorise les passages vus aux rafraîchissements précédents (3 h), `journeyStopsCache` fournit le parcours, et `findStop` retombe sur l'identifiant numérique de la gare (`LineNDirection.getOriginStopId`) quand l'estimated-timetable n'a pas renvoyé les noms d'arrêts ("Arrêt 43111"). Le parcours arrivant après les horaires au premier chargement, `TrainFragment.rebuildOngoingFromLastFetch` rejoue la construction après chaque estimated-timetable.
